describe('buckinghamshire cobrand', function() {
  ...
  beforeEach(function() {
    cy.intercept('**mapserver/bucks*Whole_Street*', {fixture: 'roads.xml'}).as('roads-layer');
    cy.intercept('**mapserver/bucks*WinterRoutes*', {fixture: 'roads.xml'}).as('winter-routes');
    cy.intercept('/report/new/ajax*').as('report-ajax');
    cy.intercept('/around\?ajax*').as('update-results');
    cy.intercept('/around/nearby*').as('around-ajax');
...
describe("Parish grass cutting category speed limit question", function() {
  ...
  function parishGrassCuttingSetup(fixtureFile, callback) {
    cy.intercept('/arcgis/services/Transport/OS_Highways_Speed/MapServer/WFSServer**', {fixture: fixtureFile}).as('speed-limits');
    cy.visit('http://buckinghamshire.localhost:3001/');
    cy.contains('Buckinghamshire');
    cy.get('[name=pc]').type('SL9 0NX');
    cy.get('[name=pc]').parents('form').submit();

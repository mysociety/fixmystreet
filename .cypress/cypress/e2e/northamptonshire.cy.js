...
it('prevents clicking unless asset selected, desktop flow', function() {
  cy.intercept('POST', '**mapserver/northamptonshire*', {fixture: 'bus_stops.xml'}).as('bus_stops-layer');
...
it('detects multiple assets at same location', function() {
  cy.intercept('POST', '**mapserver/northamptonshire*', {fixture: 'bus_stops_multiple.xml'}).as('bus_stops_multiple-layer');
  cy.intercept('/report/new/ajax*').as('report-ajax');
  cy.visit('http://northamptonshire.localhost:3001/');
  cy.get('[name=pc]').type('NN1 2NS');
  cy.get('[name=pc]').parents('form').submit();

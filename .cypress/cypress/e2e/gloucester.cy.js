describe('Gloucester cobrand', function(){
    describe('Reporting restrictions making reports on not on plot', function() {
        ...
        it('displays stopper and can not progress trying to report on not owned land', function(){
            ...
            cy.intercept('**gloucester.assets*all_plots*', {fixture: 'gloucester_plots_joint.json'}).as('gloucester_plots');
            cy.intercept('**gloucester.assets*adopted_streets*', {fixture: 'gloucester_empty.json'}).as('gloucester_streets');
        ...
        it('displays customised stopper and can not progress trying to report on GHC plot', function(){
            cy.intercept('/report/new/ajax*').as('report-ajax');
            cy.intercept('**gloucester.assets*all_plots*', {fixture: 'gloucester_plots_GCH.json'}).as('gloucester_plots');
            cy.intercept('**gloucester.assets*adopted_streets*', {fixture: 'gloucester_empty.json'}).as('gloucester_streets');
        ...
        it('does not display stopper message and allows reporting when selecting a street', function(){
            cy.intercept('/report/new/ajax*').as('report-ajax');
            cy.intercept('**gloucester.assets*all_plots*', {fixture: 'gloucester_empty.json'}).as('gloucester_plots');
            cy.intercept('**gloucester.assets*adopted_streets*', {fixture: 'gloucester_streets.json'}).as('gloucester_streets');
            cy.visit('http://gloucester.localhost:3001/report/new?longitude=-2.243133&latitude=51.865329');
            cy.pickCategory('Broken glass');
            cy.wait('@report-ajax');
            cy.wait('@gloucester_plots');

...
describe('Queen’s Park', function() {
    it('does not permit reporting within the park', function() {
        ...
        cy.intercept('**/mapserver/brent*queens_park*', {fixture: 'brent-queens_park.xml'}).as('queens_park');
...
describe('Brent road behaviour', function() {
    ...
    beforeEach(function() {
        cy.intercept('/report/new/ajax*').as('report-ajax');
        cy.intercept('**/mapserver/brent*Highways*', {fixture: 'brent-highways.xml'}).as('highways');
        cy.intercept('**/mapserver/tfl*RedRoutes*', {fixture: 'brent-tfl.xml'}).as('tfl');
        cy.viewport(480, 800);
    });
    ...
    it('allows reporting on a Brent road', function() {
        ...
        make_flytip();
        cy.contains('problem on the public highway').should('not.exist');

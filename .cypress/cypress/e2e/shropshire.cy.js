...
it('does not allow typing alphabetic characters in the phone field', function() {
    ...
    cy.intercept('**mapserver/shropshire*Street_Gazetteer*', {fixture: 'shropshire_gazetteer.xml'}).as('salop-roads-layer');
...
it('excludes one subcategory from an asset correctly', function() {
    cy.intercept('/report/new/ajax*').as('report-ajax');
    cy.intercept('**mapserver/shropshire*Street_Gazetteer*', {fixture: 'shropshire_gazetteer.xml'}).as('salop-roads-layer');
    cy.intercept('**mapserver/shropshire*Traffic_Signal_Areas*', {fixture: 'shropshire_traffic_signal_areas.xml'}).as('traffic-signal-areas');
    cy.visit('http://shropshire.localhost:3001/report/new?latitude=52.855684&longitude=-2.723877');
    cy.wait('@report-ajax');

it('allows bus stop clicking outside London', function() {
    ...
    cy.intercept('**/mapserver/tfl*busstops*', {fixture: 'tfl-bus-stops.xml'}).as('tfl-bus-stops');
    cy.intercept('**/mapserver/tfl*RedRoutes*', {fixture: 'tfl-tlrn.xml'}).as('tfl-tlrn');
...
it('shows TfL roadworks', function() {
    cy.intercept('/report/new/ajax*').as('report-ajax');
    cy.intercept('**/streetmanager*', {fixture: 'tfl-roadworks.json'}).as('roadworks');
    cy.intercept('**/mapserver/tfl*RedRoutes*', {fixture: 'tfl-tlrn.xml'}).as('tfl-tlrn');
    cy.viewport(480, 800);
...
it('shows A13 DBFO message', function() {
    cy.intercept('/report/new/ajax*').as('report-ajax');
    cy.intercept('**/mapserver/tfl*RedRoutes*', {fixture: 'tfl-tlrn.xml'}).as('tfl-tlrn');
    cy.intercept('**/mapserver/tfl*A13TLRN_DBFO*', {fixture: 'tfl-a13-dbfo-tlrn.xml'}).as('tfl-a13-dbfo-tlrn');
    cy.viewport(480, 800);

describe('National Highways tests', function() {
    it('report as defaults to body', function() {
        ...
        cy.get('#form_category_fieldset input[value="Litter"]').should('not.exist');
        cy.go('back');
    ...
    it('report as defaults to Transport Scotland', function() {
        cy.intercept('**/mapserver/highways*', {fixture: 'highways-scotland.xml'}).as('highways-tilma');
        cy.intercept('**/report/new/ajax*', {fixture: 'highways-scotland-ajax.json'}).as('report-ajax');
    ...
    it('clicking somewhere not on a Traffic Scotland road when initially centred on one still shows the report form', function() {
        ...
        cy.intercept('**/mapserver/highways*', {fixture: 'highways-scotland-dumfries.xml'}).as('highways-tilma');
        cy.intercept('**/report/new/ajax*', {fixture: 'highways-scotland-ajax.json'}).as('report-ajax');

describe('National Highways cobrand tests', function() {
    beforeEach(function() {
        cy.intercept('POST', '**/mapserver/highways', {fixture: 'highways.xml'}).as('highways-tilma');
...
describe('National Highways litter picking test', function() {
    beforeEach(function() {
        cy.intercept('POST', '**/mapserver/highways', {fixture: 'highways_a_road.xml'}).as('highways-tilma');
        cy.intercept('POST', '**/mapserver/highways?litter', {fixture: 'highways_litter.xml'}).as('highways-tilma-litter');
        cy.intercept('**/report/new/ajax*').as('report-ajax');
        cy.visit('http://highwaysengland.localhost:3001/');
        cy.contains('Go');
        cy.get('[name=pc]').type(Cypress.expose('postcode'));
...
describe('National Highways litter picking test', function() {
    beforeEach(function() {
        cy.intercept('**/mapserver/highways*', {fixture: 'highways_a_road.xml'}).as('highways-tilma');
        cy.intercept('**/report/new/ajax*', {fixture: 'highways-ajax-he-referral.json'}).as('report-ajax');
    });

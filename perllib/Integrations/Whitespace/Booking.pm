...
sub find_available_slots {
    ...
    my $c = $self->cobrand->{c};
    my $key = $self->cobrand->council_url . ":whitespace:available_bulky_slots:" . $property->{id};
    ...
    my @available_slots;
    my $slots = $self->ws->GetCollectionSlots($property->{uprn}, $window->{date_from}, $window->{date_to});
    foreach (@$slots) {

...
sub bin_services_for_address {
    ...
    $self->{c}->stash->{containers} = {
        ...
        9 => 'Communal Refuse 660L bin',
        14 => 'Communal Recycling 660L bin',
        20 => 'Paper recycling 360L bin',
        25 => 'Communal Food 240L bin',
        10 => 'Communal Refuse 1100L',
        31 => 'Communal Recycling 1280L',
        32 => 'Communal Refuse 1280L',
    };
    ...
    $self->{c}->stash->{pending_subscription} = $events->{enquiry}{1638} ? { title => 'Garden Subscription' } : undef;
    ...
    foreach (@$result) {
        ...
        foreach my $task (@$servicetasks) {
            ...
            # The most you can request is the amount you have, apart from recycling boxes, when you can request up to 5
            my $request_max = ($container == 16) ? 5 : $quantity;
...
sub waste_request_form_first_next {
    my $self = shift;
    $self->{c}->stash->{form_class} = 'FixMyStreet::App::Form::Waste::Request::Sutton';
    return 'replacement';
}

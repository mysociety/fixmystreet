...
sub send_local {
    ...
    while (my $row = $reports->fetchrow_hashref) {
        ($row->{easting}, $row->{northing}) = Utils::convert_latlon_to_en($row->{latitude}, $row->{longitude});
        my $dt = $parser->parse_timestamp( $row->{confirmed} );
        FixMyStreet->set_time_zone($dt);
        $row->{confirmed} = $dt;
    ...
    while (my $alert = $query->next) {
        ...
        foreach my $row (@reports) {
            ...
            # Ignore reports too far away (Pythagoras squared)
            next if ($row->{easting}-$easting)**2 + ($row->{northing}-$northing)**2 > $d**2;

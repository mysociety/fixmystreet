package FixMyStreet::App::Form::Licence::Hoarding;
...
# Type identifier used in URL: /licence/hoarding
sub type { 'hoarding' }

# Human-readable name for display
sub name { 'Hoarding' }

has upload_subdir => ( is => 'ro', default => 'tfl_licence_hoarding_files' );
...
has_field hoarding_length => (
    ...
    validate_method => sub {
        ...
        if ($length >= 10) {
            $saved_data->{hoarding_type} = 'Hoarding (Large)';
        } else {
            $saved_data->{hoarding_type} = 'Hoarding';
        }
    },
);

has_field hoarding_width => (
    type => 'Text',
...
# Hoarding type
# ==========================================================================
has_page hoarding_type => (
    fields => ['hoarding_type', 'hoarding_activity', 'continue'],
    title => 'Type of hoarding',
    intro => 'hoarding/type.html',
    next => 'site_pedestrian_space',
);

has_field hoarding_type => (
    type => 'Select',
    widget => 'RadioGroup',
    label => 'Hoarding type',
    required => 1,
...
sub options_hoarding_type {
    my $self = shift;
    my $data = $self->form->saved_data;
    my ($disabled_normal, $disabled_large) = (0, 0, 0);
    if ($data->{hoarding_length} >= 10) {
        $disabled_normal = 1;
    } else {
        $disabled_large = 1;
    }
    return (
        { label => 'Hoarding', value => 'Hoarding', disabled => $disabled_normal,
            hint => 'A standard hoarding less than 10 metres in length' },
        { label => 'Hoarding (Large)', value => 'Hoarding (Large)', disabled => $disabled_large,
            hint => 'Any hoarding 10 metres or greater in length' },
    );
}

has_field hoarding_activity => (
    type => 'Text',
    widget => 'Textarea',
...
# ==========================================================================
# Site Specific Information (hoarding-specific questions)
...
has_field site_within_450mm => (
    type => 'Select',
    widget => 'RadioGroup',
    label => 'Will any part of the hoarding be within 450mm of the carriageway edge?',
...
has_field site_obstruct_infrastructure => (
    type => 'Select',
    widget => 'RadioGroup',
    label => 'Will the hoarding obstruct or obscure any of the following: traffic signals, signal controllers, bus stops, pedestrian crossings, junction sight lines, road lighting columns, traffic signs, parking bays, ironwork in the highway, or other street furniture?',
...
has_page have_you_considered => (
    ...
    intro => 'hoarding/have_you_considered.html',
    next => 'terms',
);
...
# ==========================================================================
# Upload required documents (hoarding-specific)
# ==========================================================================
has_page uploads => (
    fields => [
        'upload_insurance',
...
has_field upload_site_drawing => (
    type => 'FileIdUpload',
    label => 'Hoarding layout drawing',
    tags => {
        hint => 'This must include all measurements, and insufficient drawings will not be forwarded with the application for review',
    },
    messages => {
...
has_page payment => (
    ...
    update_field_list => sub {
        ...
        my $type = $data->{hoarding_type};
        my $c = $form->{c};
        if ($type eq 'Hoarding (Large)') {
            $c->stash->{payment_link} = 'Large-Payment';
        } else {
            $c->stash->{payment_link} = 'Small-Payment';
        }
        return {};
    },
);

...
sub redirect_or_confirm_creation : Private {
    ...
    if ( $report->confirmed ) {
        ...
        if ( $c->cobrand->moniker eq 'fixmybarangay' && $c->user->from_body && $c->stash->{external_source_id}) {
            ...
            return 1;
        } elsif ( ! $c->cobrand->owns_problem( $report ) ) {
            $c->log->info("Report has been sent to a different authority than this cobrand's owner, so redirecting to confirmation page");

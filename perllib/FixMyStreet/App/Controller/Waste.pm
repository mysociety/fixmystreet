...
sub assisted : Chained('property') : Args(0) {
    my ($self, $c) = @_;
...
sub process_assisted_data : Private {
    ...
    if ($data->{outcome_choice} eq 'Approve') {
        $data->{category} = 'Request assisted collection approve';
        $data->{name} = $c->cobrand->body->comment_user->name;
        $data->{email} = $c->cobrand->body->comment_user->email;
        ...
        $c->cobrand->call_hook('_add_update_to_assisted_request', 'deny');
    }
}

...
sub uprn : Chained('/') : PathPart('hercules/uprn') : CaptureArgs(1) {
    ...
    my $property = $c->stash->{property} = FixMyStreet::Cobrand::Bromley->look_up_property($uprn);
    $c->stash->{uprn} = $uprn;
    $c->stash->{latitude} = $property->{Coordinates}{GeoPoint}{Latitude};
    $c->stash->{longitude} = $property->{Coordinates}{GeoPoint}{Longitude};

    $c->stash->{service_data} = FixMyStreet::Cobrand::Bromley->bin_services_for_address($uprn);
...
sub enquiry : Chained('uprn') : Args(0) {
    ...
    foreach (@{$contact->get_metadata_for_input}) {
        ...
        push @$field_list, "extra_$_->{code}" => { type => 'Text', label => $_->{description} },
    }

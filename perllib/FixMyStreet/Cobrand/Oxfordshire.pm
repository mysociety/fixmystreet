...
sub open311_munge_update_params {
    ...
    if ($comment->get_extra_metadata('defect_raised')) {
        ...
        my $feature;
        if ($usrn) {
            $cfg->{filter} = "<Filter><PropertyIsEqualTo><PropertyName>TYPE1_2_USRN</PropertyName><Literal>$usrn</Literal></PropertyIsEqualTo></Filter>";
            my $features = $self->_fetch_features($cfg);
            $feature = $features->[0];
        } else {
            $cfg->{filter} = "<Filter xmlns:gml=\"http://www.opengis.net/gml\"><DWithin><PropertyName>SHAPE_GEOMETRY</PropertyName><gml:Point><gml:coordinates>$e,$n</gml:coordinates></gml:Point><Distance units='m'>20</Distance></DWithin></Filter>";
            my $features = $self->_fetch_features($cfg);
            $feature = $self->_nearest_feature($cfg, $e, $n, $features);
        }
        if ($feature) {
            my $props = $feature->{properties};
            my $usrn = Utils::trim_text($props->{TYPE1_2_USRN});
            my $street = Utils::trim_text($props->{TYPE1_2_STREET_NAME});
            my $town = Utils::trim_text($props->{TYPE1_2_TOWN});

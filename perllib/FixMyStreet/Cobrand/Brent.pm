...
sub clear_cached_lookups_property {
    ...
    my $council = $self->council_url;
    foreach my $key (
        "$council:echo:look_up_property:$id",
        "$council:echo:bin_services_for_address:$id",
    ) {
...
sub clear_cached_lookups_bulky_slots {
    ...
    my $council = $self->council_url;
    foreach my $key (
        "$council:echo:available_bulky_slots:earlier:$id",
        "$council:echo:available_bulky_slots:later:$id",
        "$council:echo:bulky_event_guid:$id",
    ) {
        delete $self->{c}->session->{$key};
    }
}

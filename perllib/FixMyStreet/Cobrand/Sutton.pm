...
sub clear_cached_lookups_property {
    ...
    my $key = "sutton:echo:look_up_property:$id";
    delete $self->{c}->session->{$key};
    $key = "sutton:echo:bin_services_for_address:$id";
    delete $self->{c}->session->{$key};
}
...
around look_up_property => sub {
    ...
    if ($cfg->{nlpg} && $data->{uprn}) {
        ...
        if ($uprn_data->{results}[0]{LPI}{LOCAL_CUSTODIAN_CODE_DESCRIPTION} ne 'SUTTON') {
...
sub image_for_unit {
    my ($self, $unit) = @_;
    my $base = '/cobrands/sutton/container-images';
...
sub bin_services_for_address {
    ...
    my $calls = $echo->call_api($self->{c}, 'sutton', 'bin_services_for_address:' . $property->{id}, @to_fetch);

...
'response_template_allowed_keys' Config entry, they get replaced with the value
from Confirm. If there is no value then 'TBC' is used for date fields, or an
empty string for other fields.

The allowed field names (other than date fields) are stored in a Config object
with key 'response_template_allowed_keys', which is a hashref mapping cobrand
monikers to arrays of allowed field names.

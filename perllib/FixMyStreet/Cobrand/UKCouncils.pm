...
sub munge_report_new_bodies {
    ...
    if ( $bodies{'Traffic Scotland'} ) {
        ...
        my $fms = FixMyStreet::Cobrand::FixMyStreet->new({ c => $c });
        my $on_traffic_scotland_road = $fms->report_new_is_on_traffic_scotland_road;

        if (!$on_traffic_scotland_road) {
            %$bodies = map { $_->id => $_ } grep { $_->get_column('name') ne 'Traffic Scotland' } values %$bodies;
        }
    }

...
FixMyStreet::override_config {
   ...
   subtest 'test report a problem - waste spillage' => sub {
    ...
    subtest "Container request cancellations" => sub {
        my $cancellation_url = "/waste/12345/request/cancel/112112321";
        ...
        subtest "Link not shown if request was escalated" => sub {
            my $escalation_event = {
                Id => '112112323',
                EventTypeId => 3141, # Failure to Deliver Bags/Containers
                EventStateId => 0,
                ServiceId => 940, # Refuse
                EventDate => { DateTime => "2025-02-04T08:00:00Z" },
                Guid => 'container-escalation-event-guid',
            };
            $e->mock('GetEventsForObject', sub { [ $open_container_request_event, $escalation_event ] });
            $mech->log_in_ok($user->email);
            $mech->content_lacks($cancellation_url);
            $e->mock('GetEventsForObject', sub { [ $open_container_request_event ] });  # Revert
        };

...
FixMyStreet::override_config {
    ...
    subtest 'Request a new bin' => sub {
        ...
        is $report->detail, "One Paper and Cardboard Green Wheelie Bin (240L) to deliver\n\nOne Paper and Cardboard Green Wheelie Bin (240L) to remove\n\n2 Example Street, Sutton, SM1 1AA\n\nReason: Damaged";
    ...
    subtest 'Report a new recycling raises a bin delivery request' => sub {
        ...
        $mech->submit_form_ok({ with_fields => { name => 'Bob Marge', email => $user->email }});
        $mech->content_contains('Missing (1 to deliver)');
        $mech->submit_form_ok({ with_fields => { process => 'summary' } });
        $mech->content_contains('request has been sent');
        my $report = FixMyStreet::DB->resultset("Problem")->order_by('-id')->first;
        is $report->get_extra_field_value('uprn'), 1000000002;
        is $report->detail, "One Mixed Recycling Green Box (55L) to deliver\n\n2 Example Street, Sutton, SM1 1AA\n\nReason: Missing";
    ...
    subtest 'Fortnightly collection can request a blue stripe bag' => sub {
        ...
        my $report = FixMyStreet::DB->resultset("Problem")->order_by('-id')->first;
        is $report->get_extra_field_value('uprn'), 1000000002;
        is $report->detail, "One Mixed Recycling Blue Striped Bag to deliver\n\n2 Example Street, Sutton, SM1 1AA\n\nReason: Additional bag required";
        is $report->category, 'Request new container';

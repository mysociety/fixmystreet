...
FixMyStreet::override_config {
    ...
    subtest 'No reporting/requesting if open request' => sub {
        ...
        #$mech->content_lacks(qr/A mixed recycling \(cans, plastics &amp; glass\) collection was reported as missed\s+on Saturday, 10 September/);
   ...
   subtest 'test report a problem - waste spillage' => sub {
    ...
    subtest 'No spillage report for open request in same service' => sub {
        ...
        $mech->content_like(qr/You have already reported a waste spillage/s);
        $mech->back;
    ...
    subtest 'Escalations of missed collections' => sub {
        subtest 'No missed collection' => sub {
            for my $date ('2022-09-10T19:00:00Z', '2022-09-13T19:00:00Z', '2022-09-15T17:00:00Z', '2022-09-15T19:00:00Z') {
                ...
                $mech->content_lacks('Report no action on complaint', 'No escalation option available without missed collection reported');
            }
        };
        ...
        subtest 'Open missed collection' => sub {
            ...
            $mech->follow_link_ok( { url_regex => qr/service_id=940/}, 'Follow "Report a problem" link for Non-Recyclable Waste collection' );
            $mech->content_contains('class="govuk-heading-xl">Report a problem', 'On the report a problem page');
            $mech->content_lacks('Report no action on complaint', 'No escalation option available due to date');

            set_fixed_time('2022-09-13T19:00:00Z');
            $mech->get_ok('/waste/12345');
            $mech->follow_link_ok( { url_regex => qr/service_id=940/}, 'Follow "Report a problem" link for Non-Recyclable Waste collection' );
            $mech->content_contains('class="govuk-heading-xl">Report a problem', 'On the report a problem page');
            $mech->content_contains('Report no action on complaint', 'Escalation option available');
            ...
            $mech->content_contains('class="govuk-heading-xl">Report a problem', 'On the report a problem page');
            $mech->content_contains('Report no action on complaint', 'Escalation option available');
            $mech->content_contains('please report the problem here', 'Allows escalating');

            set_fixed_time('2022-09-15T19:00:00Z');
            $mech->get_ok('/waste/12345');
            $mech->follow_link_ok( { url_regex => qr/service_id=940/}, 'Follow "Report a problem" link for Non-Recyclable Waste collection' );
            $mech->content_lacks('Report no action on complaint', 'Escalation option not available');
        };
        ...
        subtest 'Completed missed collection - no escalation' => sub {
            ...
            for my $date ('2022-09-10T19:00:00Z', '2022-09-13T19:00:00Z', '2022-09-15T17:00:00Z', '2022-09-15T19:00:00Z' ) {
                ...
                $mech->get_ok('/waste/12345');
                $mech->follow_link_ok( { url_regex => qr/service_id=940/}, 'Follow "Report a problem" link for Non-Recyclable Waste collection' );
                $mech->content_lacks('Report no action on complaint', 'No option to escalate missed collection');
            }
        };
        ...
        subtest 'Existing escalation event' => sub {
            ...
            $mech->get_ok('/waste/12345');
            $mech->follow_link_ok( { url_regex => qr/service_id=940/}, 'Follow "Report a problem" link for Non-Recyclable Waste collection' );
            $mech->content_like(qr/redirect-missed" aria-describedby="category-item-hint"\s+disabled/s, "Can not report missed as already reported");
            $mech->content_contains('You have reported this collection as missed', "Message to resident displayed");
        };
        ...
        subtest "Open request not escalated but outside window; can't escalate" => sub {
            foreach my $config ((
                ...
                subtest $config->{label} => sub {
                    ...
                    $mech->content_contains('A non-recyclable refuse container request was made on Monday, 3 February');
                    $mech->follow_link_ok( { url_regex => qr/service_id=940/}, 'Follow "Report a problem" link for Non-Recyclable Waste collection' );
                    $mech->content_lacks('bags have not been delivered');
                };
            }
        };
        ...
        subtest "Open request not escalated and inside window; can escalate" => sub {
            foreach my $config ((
                ...
                subtest $config->{label} => sub {
                    ...
                    $mech->content_contains('please report the problem here', 'hint for user correct');

...
subtest 'Scaffold form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        # Activity page
        $mech->submit_form_ok({ with_fields => {
            activity => 'Spring bulbs',
            description => 'Daffofils mostly',
        }}, 'Activity page');

        # Site specific pages (one question per page)
        $mech->submit_form_ok({ with_fields => {
            footway_incursion => 'None',
            site_adequate_space => 'Yes'
        }}, 'Footway page');

        $mech->submit_form_ok({ with_fields => {
            carriageway_incursion=> 'None',
        ...
        $mech->submit_form_ok({ with_fields => {
            ...
            upload_specification => [ $sample_pdf, undef, Content_Type => 'application/pdf' ],
        }}, 'Upload page');
        ...
        for my $field (qw(upload_insurance upload_rams upload_site_drawing upload_specification)) {

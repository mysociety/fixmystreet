...
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Landscaping/Planting licence',
...
subtest 'Scaffold form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        $mech->get_ok('/licence/landscaping');
        ...
        # Applicant page
        $mech->submit_form_ok({ with_fields => {
            organisation => 'Test Landscaping Ltd',
        ...
        # Activity page
        $mech->submit_form_ok({ with_fields => {
            landscaping_activity => 'Spring bulbs',
            landscaping_description => 'Daffofils mostly',
        }}, 'Activity page');

        # Site specific pages (one question per page)
        $mech->submit_form_ok({ with_fields => {
            landscaping_footway_incursion => 'None',
            site_adequate_space => 'Yes'
        }}, 'Footway page');

        $mech->submit_form_ok({ with_fields => {
            landscaping_carriageway_incursion=> 'None',
        ...
        $mech->submit_form_ok({ with_fields => {
            ...
            upload_landscaping_site_drawing => [ $sample_pdf, undef, Content_Type => 'application/pdf' ],
            upload_landscaping_specification => [ $sample_pdf, undef, Content_Type => 'application/pdf' ],
        }}, 'Upload page');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Landscaping/Planting licence' })
        ...
        like $detail, qr/\[Location of the Landscaping\/Planting\]/, 'Detail contains Location section header';
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl_licence_landscaping_files")->absolute(FixMyStreet->path_to());
        ...
        for my $field (qw(upload_insurance upload_rams upload_landscaping_site_drawing upload_landscaping_specification)) {

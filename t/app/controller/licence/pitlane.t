...
# Create the category for pit lane licences
  ...
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        COBRAND_FEATURES => {
            ...
            licencing_payment_links => { tfl => { 'pit-lane' => { minor => 'MINOR' } } },
        },
        PHOTO_STORAGE_OPTIONS => { UPLOAD_DIR => $UPLOAD_DIR },
    }, sub {
        $mech->get_ok('/licence/pit-lane');
        ...
        # Applicant page
        $mech->submit_form_ok({ with_fields => {
            organisation => 'Test Pit Lane Ltd',
            job_title => 'Pitter',
            name => 'Test Person',
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Pit Lane guidance notes and terms & conditions - March 2026');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Pit Lane licence' })
        ...
        for my $field (qw(upload_insurance upload_rams upload_traffic_management)) {

...
# Create the category for statues/artworks licences
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Statues/Artworks licence',
    email => 'licence@tfl.gov.uk'
);

subtest 'Statues/Artworks form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        $mech->get_ok('/licence/statues-artworks');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Statues/Artworks licence' })
        ...
        like $detail, qr/\[Location of the statue\/artwork\]/, 'Detail contains Location section header';
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl_licence_statues-artworks_files")->absolute(FixMyStreet->path_to());
        ...
        for my $field (qw(upload_insurance upload_rams upload_site_drawing upload_illustration)) {

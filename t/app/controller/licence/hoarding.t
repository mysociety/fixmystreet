...
# Create the category for hoarding licences
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Hoarding licence',
    email => 'licence@tfl.gov.uk.example.org'
);

subtest 'Hoarding form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        # Applicant page
        $mech->submit_form_ok({ with_fields => {
            organisation => 'Test Hoarding Ltd',
            job_title => 'Hoarder',
            name => 'Test Person',
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Hoarding guidance notes and terms & conditions - March 2026');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Hoarding licence' })
        ...
        like $detail, qr/\[Location of the hoarding\]/, 'Detail contains Location section header';
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl-licence-hoarding")->absolute(FixMyStreet->path_to());

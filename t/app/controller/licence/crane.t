...
# Create the category for crane licences
  ...
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        COBRAND_FEATURES => {
            ...
            licencing_payment_links => { tfl => { crane => { oversail => 'OVERSAIL' } } },
        },
        PHOTO_STORAGE_OPTIONS => { UPLOAD_DIR => $UPLOAD_DIR },
    }, sub {
        $mech->get_ok('/licence/crane');
        ...
        $mech->content_like(qr/name="crane_type" value="Crane \(Lift\)"\s+checked/);
        $mech->back;
        $mech->submit_form_ok({ with_fields => {
            lifting => 'No'
        }});
        $mech->content_unlike(qr/name="crane_type" value="Crane \(Lift\)"\s+checked/);
        $mech->submit_form_ok({ with_fields => {
            crane_type => 'Crane (Oversail)',
        }});
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Crane guidance notes and terms & conditions - March 2026');
        ...
        like $detail, qr/\[Location of the crane\]/, 'Detail contains Location section header';

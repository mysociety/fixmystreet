...
# Create the category for Temporary Traffic Signs licences
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Temporary Traffic Signs licence',
    email => 'licence@tfl.gov.uk.example.org'
);

subtest 'Temporary Traffic Signs form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        COBRAND_FEATURES => {
            ...
            licencing_payment_links => { tfl => { 'temporary-traffic-signs' => { four => 'LINK' } } },
        },
        PHOTO_STORAGE_OPTIONS => { UPLOAD_DIR => $UPLOAD_DIR },
    }, sub {
        $mech->get_ok('/licence/temporary-traffic-signs');
        ...
        # Applicant page
        $mech->submit_form_ok({ with_fields => {
            organisation => 'Test Traffic Signs Ltd',
            job_title => 'Temporary Sign Enjoyer',
            name => 'Test Person',
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Temporary Traffic Signs guidance notes and terms & conditions - March 2026');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Temporary Traffic Signs licence' })
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl-licence-temporary-traffic-signs")->absolute(FixMyStreet->path_to());

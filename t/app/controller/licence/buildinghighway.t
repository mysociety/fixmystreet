...
# Create the category for licences
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Building over the Highway licence',
    email => 'licence@tfl.gov.uk.example.org'
);

subtest 'Building over Highway form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        $mech->get_ok('/licence/building-over-highway');
        ...
        # Applicant page
        $mech->submit_form_ok({ with_fields => {
            organisation => 'Test Building Ltd',
            job_title => 'Builder',
            name => 'Test Person',
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Building over the Highway guidance notes and terms & conditions - March 2026');
        ...
        $mech->submit_form_ok({ with_fields => {
            ...
            upload_planning_consent => [ $sample_pdf, undef, Content_Type => 'application/pdf' ],
        }});
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Building over the Highway licence' })
        ...
        like $detail, qr/\[Location of building over the highway\]/, 'Detail contains Location section header';
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl-licence-building-over-highway")->absolute(FixMyStreet->path_to());
        ...
        for my $field (qw(upload_insurance upload_rams upload_site_drawing upload_planning_consent)) {

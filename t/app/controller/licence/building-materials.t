...
# Create the category for building materials licences
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Building materials licence',
    email => 'licence@tfl.gov.uk.example.org'
);

subtest 'Building materials form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        $mech->get_ok('/licence/building-materials');
        ...
        $mech->submit_form_ok({ with_fields => {
            ...
            building_materials_stored => 'bagged sand',
        }});
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Building materials guidance notes and terms & conditions - March 2026');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Building materials licence' })
        ...
        like $detail, qr/\[Location of building materials\]/, 'Detail contains Location section header';
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl-licence-building-materials")->absolute(FixMyStreet->path_to());

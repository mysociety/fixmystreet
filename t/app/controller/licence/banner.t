...
subtest 'Banner form submission - smoke test' => sub {
    ...
    FixMyStreet::override_config {
        ...
        # Banner activity
        $mech->submit_form_ok({ with_fields => {
            banner_type => 'Placard on lamppost',
            banner_content => '"BIG SALE"',
        }});
        $mech->submit_form_ok({ with_fields => {
            banner_installation => 'Tiny little ladders',
        }});
        ...
        $mech->submit_form_ok({ with_fields => {
            ...
            footway_low_headroom => 'No',
        }});
        $mech->submit_form_ok({ with_fields => {
            carriageway_incursion => 'No carriageway incursion',
            carriageway_low_headroom => 'No',
        }});
        $mech->submit_form_ok({ with_fields => {
            site_ad_compliance => 'Yes',
            site_lighting_columns => 'No',
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl_licence_banner_files")->absolute(FixMyStreet->path_to());

...
# Create the category for lights in trees licences
my $contact = $mech->create_contact_ok(
    body_id => $body->id,
    category => 'Lights in Trees licence',
    email => 'licence@tfl.gov.uk.example.org'
);

subtest 'Lights in trees form submission - smoke test' => sub {
  my $UPLOAD_DIR = tempdir( CLEANUP => 1 );
    FixMyStreet::override_config {
        ...
        COBRAND_FEATURES => {
            ...
            licencing_payment_links => { tfl => { 'lights-in-trees' => 'LINK' } },
        },
        PHOTO_STORAGE_OPTIONS => { UPLOAD_DIR => $UPLOAD_DIR },
    }, sub {
        $mech->get_ok('/licence/lights-in-trees');
        ...
        # Applicant page
        $mech->submit_form_ok({ with_fields => {
            organisation => 'Test Lighting Ltd',
            job_title => 'Lights-in-trees enjoyer',
            name => 'Test Person',
        ...
        $mech->submit_form_ok({ with_fields => {
            installation_method => 'Strips of LEDs',
        }});

        # Activity page
        $mech->submit_form_ok({ with_fields => {
            activity => 'Temporary lighting',
        }});
        ...
        $mech->current_form->find_input('terms_accepted', undef, 1)->value('Lights in Trees guidance notes and terms & conditions - March 2026');
        ...
        my $problem = FixMyStreet::DB->resultset('Problem')
            ->search({ category => 'Lights in Trees licence' })
        ...
        like $detail, qr/\[Location of the lights in trees\]/, 'Detail contains Location section header';
        ...
        my $upload_dir = path($UPLOAD_DIR, "tfl-licence-lights-in-trees")->absolute(FixMyStreet->path_to());

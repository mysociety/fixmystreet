...
FixMyStreet::override_config {
    ...
    subtest 'Details with approval' => sub {
        for my $fields
            (
                {request_property_type => '2', request_reason_refuse => '1', request_reason_refuse_size => '3'}
            )
        {
            $mech->submit_form_ok({
                with_fields => $fields
            ...
            # Should have option to change application data here!
            # Need to set up the questions better
    ...
    subtest 'Automatically calculated response' => sub {
        ...
        for my $fields
            (
                {request_property_type => '1', request_reason_refuse => '1', request_reason_refuse_size => '3'}
            )
            {
                $mech->submit_form_ok({
                    with_fields => $fields

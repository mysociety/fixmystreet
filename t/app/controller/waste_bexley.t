...
my $assisted_collection_approval = $mech->create_contact_ok(
    body => $body,
    category => 'Request assisted collection approve',
...
FixMyStreet::override_config {
    ...
    subtest 'Request assisted collection staff field' => sub {
        ...
        $mech->submit_form_ok({ form_number => 3 });
        $mech->content_contains('You can respond to this with this <a href="http://bexley.example.org/waste/10006/assisted">form</a>', "Link for staff to approve/deny request present");
    ...
    subtest 'Request assisted collection approval' => sub {
        $mech->get_ok('/waste/10006/assisted');
        ...
        my $report = FixMyStreet::DB->resultset('Problem')->search( { category => 'Request assisted collection' } )->first;
        is $report->state, 'fixed - council', "Request report marked fixed";

        my $open311_report = FixMyStreet::DB->resultset('Problem')->search( { category => 'Request assisted collection approve' } )->first;

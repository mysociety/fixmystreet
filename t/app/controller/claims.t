...
FixMyStreet::override_config {
    ...
    subtest 'Report new vehicle claim, report id known' => sub {
        ...
        $mech->submit_form_ok({ with_fields => { what_cause => 'bollard', aware => 'Yes', where_cause => 'bridge', describe_cause => 'a cause', photos => 'phoot!' } }, "cause screen");
        $mech->submit_form_ok({ with_fields => { make => 'a car', registration => 'rego!', mileage => '20', v5 => 'v5', v5_in_name => 'Yes', insurer_address => 'insurer address', damage_claim => 'No', vat_reg => 'No' } }, "car details");
        $mech->submit_form_ok({ with_fields => { vehicle_damage => 'the car was broken', vehicle_photos => 'car photos', vehicle_receipts => 'receipt photos', tyre_damage => 'Yes', tyre_mileage => 20, tyre_receipts => 'tyre receipts' } }, "damage details");
    ...
    subtest 'Report new vehicle claim, report fixed' => sub {
        ...
        $mech->submit_form_ok({ with_fields => { weather => 'sunny', direction => 'east', details => 'some details', in_vehicle => 'Yes', speed => '20mph', actions => 'an action' } }, 'incident details');
        $mech->submit_form_ok({ with_fields => { witnesses => 'Yes', witness_details => 'some witnesses', report_police => 'Yes', incident_number => 23 } }, 'witnesses etc');
        $mech->submit_form_ok({ with_fields => { what_cause => 'other', what_cause_other => '', aware => 'Yes', where_cause => 'bridge', describe_cause => 'a cause', photos => 'phoot!' } }, 'cause details');
        $mech->content_contains('Other cause field is required');
        $mech->submit_form_ok({ with_fields => { what_cause => 'other', what_cause_other => 'Duck', aware => 'Yes', where_cause => 'bridge', describe_cause => 'a cause', photos => 'phoot!' } }, 'cause details');
        $mech->submit_form_ok({ with_fields => { make => 'a car', registration => 'rego!', mileage => '20', v5 => 'v5', v5_in_name => 'Yes', insurer_address => 'insurer address', damage_claim => 'No', vat_reg => 'No' } }, 'vehicle details');
        $mech->submit_form_ok({ with_fields => { vehicle_damage => 'the car was broken', vehicle_photos => 'car photos', vehicle_receipts => 'receipt photos', tyre_damage => 'Yes', tyre_mileage => 20, tyre_receipts => 'tyre receipts' } }, 'damage details');

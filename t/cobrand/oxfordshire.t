...
FixMyStreet::override_config {
    ...
    subtest 'extra data sent with defect update' => sub {
        ...
        $cbr->mock('_fetch_features', sub {
            my ($self, $cfg, $x, $y) = @_;
            if ($cfg->{filter} =~ /<PropertyIsEqualTo><PropertyName>TYPE1_2_USRN<\/PropertyName><Literal>(.*?)</) {
                [ {
                    type => 'Feature',
                    geometry => { type => 'LineString', coordinates => [ [ 1, 2 ], [ 3, 4 ] ] },
                    properties => { TYPE1_2_USRN => $1, TYPE1_2_STREET_NAME => 'Street Avenue', TYPE1_2_TOWN => 'Town' },
                } ]
            } else {
                [ {
                    type => 'Feature',
                    geometry => { type => 'LineString', coordinates => [ [ 1, 2 ], [ 3, 4 ] ] },
                    properties => { TYPE1_2_USRN => 13579, TYPE1_2_STREET_NAME => 'Street Avenue', TYPE1_2_TOWN => 'Town' },
                } ]
            }
        });
